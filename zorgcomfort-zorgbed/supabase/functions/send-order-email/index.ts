...
serve(async (req: Request) => {
  ...
  try {
    ...
    const recipientEmail = Deno.env.get('RECIPIENT_EMAIL') ?? 'info@zorgcomfort.nl';
    // Afzender instelbaar via env (moet een geverifieerd Resend-domein zijn).
    const fromAddress = Deno.env.get('EMAIL_FROM') ?? 'ZorgComfort Zorgbedden <noreply@zorgcomfort.nl>';
